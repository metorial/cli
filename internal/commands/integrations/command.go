...
type magicMcpServerProviderListOutput struct {
	Items []magicMcpServerProviderListOutputItem `json:"items"`
}

type magicMcpServerProviderListOutputItem struct {
...
func listMagicMcpServerProviders(runtime config.Runtime, sdk *metorial.MetorialSdk, magicMcpServerID string) (*magicMcpServerProviderListOutput, error) {
	...
	var providers magicMcpServerProviderListOutput
...
func createMagicMcpServerProvider(runtime config.Runtime, sdk *metorial.MetorialSdk, magicMcpServerID string, body map[string]any) (map[string]any, error) {
	payload, err := json.Marshal(body)
	if err != nil {
		return nil, fmt.Errorf("metorial: failed to encode magic MCP provider assignment: %w", err)
	}

	response, err := consumerSDKFetch(runtime, sdk, "POST", fmt.Sprintf("/magic-mcp-servers/%s/providers", strings.TrimSpace(magicMcpServerID)), payload)
	if err != nil {
		return nil, err
	}

	var providerAssignment map[string]any
...
func renderSetupResult(writer io.Writer, features terminal.Features, setupSession *setupsessions.ProviderDeploymentsSetupSessionsGetOutput, listing *providerlistings.ProviderListingsGetOutput, provider *providers.ProvidersGetOutput, server *magicmcpservers.MagicMcpServersCreateOutput, sessionTemplateProvider map[string]any, tips []string) error {
	colors := terminal.NewColorizer(features)
	title := server.Id
	if name := optionalString(server.Name); name != "" {
		title = name
